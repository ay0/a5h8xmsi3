...
void CEsp::DrawPlayer(IClientEntity* pEntity, player_info_t pinfo)
{
	...
	// warning = no error
		...
		if (Options::Menu.VisualsTab.OptionsHealth.GetIndex() == 4) // having a mental stroke trying to figure out XD
...
void CEsp::DrawInfo(IClientEntity* pEntity, Box size)
{
	...
	if (Options::Menu.VisualsTab.OptionsInfo.GetState())
	{
		...
		if (pEntity->ArmorValue() > 0)
		{
			...
			if (pEntity->HasHelmet())
				Render::Text(size.x + size.w + 1, size.y + 0, Color(255, 255, 255, 255), Render::Fonts::WeaponIcon, "HK"); // WeaponIcon meme 
			else
				Render::Text(size.x + size.w + 1, size.y + 0, Color(210, 210, 210, 255), Render::Fonts::WeaponIcon, "Kevlar");
		}
		else
			Render::Text(size.x + size.w + 1, size.y + 0, Color(0, 210, 255, 255), Render::Fonts::WeaponIcon, "No Armor");
	...
	if (Options::Menu.VisualsTab.OptionsInfo.GetState())
	{
		if (pEntity->IsScoped())
		{
			...
			Render::Text(size.x + size.w + 1, size.y + 20, Color(0, 200, 255, 255), Render::Fonts::WeaponIcon, "IsScoped");
		}
